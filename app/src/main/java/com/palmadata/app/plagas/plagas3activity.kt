...
import com.palmadata.app.databinding.ActivityPlagas3Binding

class Plagas3Activity : AppCompatActivity() {
    private lateinit var binding: ActivityPlagas3Binding
    private var valorActual = ""
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityPlagas3Binding.inflate(layoutInflater)
        ...
        binding.btnAccion.setOnClickListener {
            if (valorActual.length != 6) { mostrarError("El CENSO debe tener exactamente 6 dígitos"); return@setOnClickListener }
            startActivity(Intent(this, Plagas4Activity::class.java).also {
                ...
                it.putExtra("lectura", valorActual)
            })
        }
    }
    private fun setupTeclado() {
        ...
        botones.forEach { (btn, v) -> btn.setOnClickListener { if (valorActual.length < 6) { valorActual += v; actualizarDisplay() } } }
