package com.palmadata.app.super_poli

import android.content.Intent
import android.os.Bundle
import androidx.appcompat.app.AppCompatActivity
import androidx.recyclerview.widget.LinearLayoutManager
import com.palmadata.app.databinding.ActivitySuperPoli0Binding
import com.palmadata.app.ui.WorkerAdapter
import com.palmadata.app.utils.DatabaseHelper

class SuperPoli0Activity : AppCompatActivity() {
    private lateinit var binding: ActivitySuperPoli0Binding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySuperPoli0Binding.inflate(layoutInflater)
        setContentView(binding.root)
        val plantaciones = DatabaseHelper.getInstance(this).getPlantaciones()
        val adapter = WorkerAdapter { nombre ->
            val p = plantaciones.first { it.second == nombre }
            startActivity(Intent(this, SuperPoli1Activity::class.java).also {
                it.putExtra("plantacion_id", p.first)
                it.putExtra("plantacion_nombre", p.second)
            })
        }
        binding.rvPlantaciones.layoutManager = LinearLayoutManager(this)
        binding.rvPlantaciones.adapter = adapter
        adapter.submitList(plantaciones.map { it.second })
    }
}